// Auto-generated by codegen - Skygen
// https://cloudflavor.io/projects/skygen
//
// Copyright 2025 Cloudflavor GmbH
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use crate::models::scaleway_vpc_v2_route::ScalewayVpcV2Route;
use crate::{ApiClient, ApiRequestBuilder, ApiResult};
use reqwest::Method;

#[derive(Debug)]
pub struct CreateRouteRequest<'a> {
    builder: ApiRequestBuilder<'a, ScalewayVpcV2Route>,
}

impl<'a> CreateRouteRequest<'a> {
    pub fn new(api: &'a ApiClient) -> Self {
        let builder = ApiRequestBuilder::new(api, Method::POST, "vpc/v2/regions/{region}/routes")
            .require_path("region")
            .require_body();

        Self { builder }
    }
    pub fn with_region(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("region", value);
        self
    }
    pub fn with_body(
        mut self,
        body: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.builder = self.builder.json_body(body).expect("body serialization");
        self
    }
    pub async fn send(self) -> ApiResult<ScalewayVpcV2Route> {
        self.builder.send().await
    }
}
/// Create a Route
///
/// Create a new custom Route.
///
/// **HTTP Method:** `POST`
/// **Path:** `/vpc/v2/regions/{region}/routes`
///
/// **Parameters**
/// - `region` (path, required)
///
/// # Example
/// ```no_run
/// use vpc::{ ApiClient, apis::routes };
/// let api = ApiClient::builder("https://api.example.com").build().expect("client");
/// # let body: std::collections::BTreeMap<String, serde_json::Value> = todo!();
/// let response = create_route(&api)
///     .with_region("region")
///     .with_body(body)
///     .send()
///     .await?;
/// ```
pub fn create_route(api: &ApiClient) -> CreateRouteRequest<'_> {
    CreateRouteRequest::new(api)
}

#[derive(Debug)]
pub struct RouteRequest<'a> {
    builder: ApiRequestBuilder<'a, ScalewayVpcV2Route>,
}

impl<'a> RouteRequest<'a> {
    pub fn new(api: &'a ApiClient) -> Self {
        let builder = ApiRequestBuilder::new(
            api,
            Method::GET,
            "vpc/v2/regions/{region}/routes/{route_id}",
        )
        .require_path("region")
        .require_path("route_id");

        Self { builder }
    }
    pub fn with_region(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("region", value);
        self
    }
    pub fn with_route_id(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("route_id", value);
        self
    }
    pub async fn send(self) -> ApiResult<ScalewayVpcV2Route> {
        self.builder.send().await
    }
}
/// Get a Route
///
/// Retrieve details of an existing Route, specified by its Route ID.
///
/// **HTTP Method:** `GET`
/// **Path:** `/vpc/v2/regions/{region}/routes/{route_id}`
///
/// **Parameters**
/// - `region` (path, required)
/// - `route_id` (path, required)
///
/// # Example
/// ```no_run
/// use vpc::{ ApiClient, apis::routes };
/// let api = ApiClient::builder("https://api.example.com").build().expect("client");
/// let response = route(&api)
///     .with_region("region")
///     .with_route_id("route_id")
///     .send()
///     .await?;
/// ```
pub fn route(api: &ApiClient) -> RouteRequest<'_> {
    RouteRequest::new(api)
}

#[derive(Debug)]
pub struct RouteDeleteRequest<'a> {
    builder: ApiRequestBuilder<'a, serde_json::Value>,
}

impl<'a> RouteDeleteRequest<'a> {
    pub fn new(api: &'a ApiClient) -> Self {
        let builder = ApiRequestBuilder::new(
            api,
            Method::DELETE,
            "vpc/v2/regions/{region}/routes/{route_id}",
        )
        .require_path("region")
        .require_path("route_id");

        Self { builder }
    }
    pub fn with_region(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("region", value);
        self
    }
    pub fn with_route_id(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("route_id", value);
        self
    }
    pub async fn send(self) -> ApiResult<serde_json::Value> {
        self.builder.send().await
    }
}
/// Delete a Route
///
/// Delete a Route specified by its Route ID.
///
/// **HTTP Method:** `DELETE`
/// **Path:** `/vpc/v2/regions/{region}/routes/{route_id}`
///
/// **Parameters**
/// - `region` (path, required)
/// - `route_id` (path, required)
///
/// # Example
/// ```no_run
/// use vpc::{ ApiClient, apis::routes };
/// let api = ApiClient::builder("https://api.example.com").build().expect("client");
/// let response = route_delete(&api)
///     .with_region("region")
///     .with_route_id("route_id")
///     .send()
///     .await?;
/// ```
pub fn route_delete(api: &ApiClient) -> RouteDeleteRequest<'_> {
    RouteDeleteRequest::new(api)
}

#[derive(Debug)]
pub struct UpdateRouteRequest<'a> {
    builder: ApiRequestBuilder<'a, ScalewayVpcV2Route>,
}

impl<'a> UpdateRouteRequest<'a> {
    pub fn new(api: &'a ApiClient) -> Self {
        let builder = ApiRequestBuilder::new(
            api,
            Method::PATCH,
            "vpc/v2/regions/{region}/routes/{route_id}",
        )
        .require_path("region")
        .require_path("route_id")
        .require_body();

        Self { builder }
    }
    pub fn with_region(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("region", value);
        self
    }
    pub fn with_route_id(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("route_id", value);
        self
    }
    pub fn with_body(
        mut self,
        body: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.builder = self.builder.json_body(body).expect("body serialization");
        self
    }
    pub async fn send(self) -> ApiResult<ScalewayVpcV2Route> {
        self.builder.send().await
    }
}
/// Update Route
///
/// Update parameters of the specified Route.
///
/// **HTTP Method:** `PATCH`
/// **Path:** `/vpc/v2/regions/{region}/routes/{route_id}`
///
/// **Parameters**
/// - `region` (path, required)
/// - `route_id` (path, required)
///
/// # Example
/// ```no_run
/// use vpc::{ ApiClient, apis::routes };
/// let api = ApiClient::builder("https://api.example.com").build().expect("client");
/// # let body: std::collections::BTreeMap<String, serde_json::Value> = todo!();
/// let response = update_route(&api)
///     .with_region("region")
///     .with_route_id("route_id")
///     .with_body(body)
///     .send()
///     .await?;
/// ```
pub fn update_route(api: &ApiClient) -> UpdateRouteRequest<'_> {
    UpdateRouteRequest::new(api)
}
