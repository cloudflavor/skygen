// Auto-generated by codegen - Skygen
// https://cloudflavor.io/projects/skygen
//
// Copyright 2025 Cloudflavor GmbH
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use crate::models::scaleway_datawarehouse_v1beta1_endpoint::ScalewayDatawarehouseV1beta1Endpoint;
use crate::{ApiClient, ApiRequestBuilder, ApiResult};
use reqwest::Method;

#[derive(Debug)]
pub struct CreateEndpointRequest<'a> {
    builder: ApiRequestBuilder<'a, ScalewayDatawarehouseV1beta1Endpoint>,
}

impl<'a> CreateEndpointRequest<'a> {
    pub fn new(api: &'a ApiClient) -> Self {
        let builder = ApiRequestBuilder::new(
            api,
            Method::POST,
            "datawarehouse/v1beta1/regions/{region}/endpoints",
        )
        .require_path("region")
        .require_body();

        Self { builder }
    }
    pub fn with_region(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("region", value);
        self
    }
    pub fn with_body(
        mut self,
        body: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.builder = self.builder.json_body(body).expect("body serialization");
        self
    }
    pub async fn send(self) -> ApiResult<ScalewayDatawarehouseV1beta1Endpoint> {
        self.builder.send().await
    }
}
/// Create a new endpoint for a deployment
///
/// **HTTP Method:** `POST`
/// **Path:** `/datawarehouse/v1beta1/regions/{region}/endpoints`
///
/// **Parameters**
/// - `region` (path, required)
///
/// # Example
/// ```no_run
/// use datawarehouse::{ ApiClient, apis::endpoint_management };
/// let api = ApiClient::builder("https://api.example.com").build().expect("client");
/// # let body: std::collections::BTreeMap<String, serde_json::Value> = todo!();
/// let response = create_endpoint(&api)
///     .with_region("region")
///     .with_body(body)
///     .send()
///     .await?;
/// ```
pub fn create_endpoint(api: &ApiClient) -> CreateEndpointRequest<'_> {
    CreateEndpointRequest::new(api)
}

#[derive(Debug)]
pub struct DeleteRequest<'a> {
    builder: ApiRequestBuilder<'a, serde_json::Value>,
}

impl<'a> DeleteRequest<'a> {
    pub fn new(api: &'a ApiClient) -> Self {
        let builder = ApiRequestBuilder::new(
            api,
            Method::DELETE,
            "datawarehouse/v1beta1/regions/{region}/endpoints/{endpoint_id}",
        )
        .require_path("region")
        .require_path("endpoint_id");

        Self { builder }
    }
    pub fn with_region(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("region", value);
        self
    }
    pub fn with_endpoint_id(mut self, value: impl Into<String>) -> Self {
        self.builder = self.builder.path_param("endpoint_id", value);
        self
    }
    pub async fn send(self) -> ApiResult<serde_json::Value> {
        self.builder.send().await
    }
}
/// Delete an endpoint from a deployment
///
/// **HTTP Method:** `DELETE`
/// **Path:** `/datawarehouse/v1beta1/regions/{region}/endpoints/{endpoint_id}`
///
/// **Parameters**
/// - `region` (path, required)
/// - `endpoint_id` (path, required)
///
/// # Example
/// ```no_run
/// use datawarehouse::{ ApiClient, apis::endpoint_management };
/// let api = ApiClient::builder("https://api.example.com").build().expect("client");
/// let response = delete(&api)
///     .with_region("region")
///     .with_endpoint_id("endpoint_id")
///     .send()
///     .await?;
/// ```
pub fn delete(api: &ApiClient) -> DeleteRequest<'_> {
    DeleteRequest::new(api)
}
